...
public abstract class StreamServer {
	...
	private class HTTPSession extends Thread {
		 ...
		 */
		private void sendResponse(Socket socket, String status, String mime,
				Properties header, StreamSource data) {
			try {
				if (status == null)
				...
				if (data != null) {
					// long pending = data.available(); // This is to support
					// partial sends, see serveFile()
					byte[] buff = new byte[8192];
					int read = 0;
					while ((read = data.read(buff)) > 0) {
						// Log.d("Explorer", "Read: "+ read +", pending: "+
						// data.available());
	...
	//
	// // Calculate etag
	// String etag = Integer.toHexString((f.getAbsolutePath() + f.lastModified()
	// + "" + f.length()).hashCode());
	//
